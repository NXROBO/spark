...
typedef struct XnCommandAHB
{
  ...
  uint32_t value;         // For read requests, this is where the actual value will be
                          // filled. For write requests, the value to write.
} XnCommandAHB;
...
typedef struct XnCommandI2C
{
  ...
  uint32_t mask;         // For write request - a mask to be applied to the value. For
                         // read requests - ignored.
  uint32_t value;        // For write request - the value to be written. For read
                         // requests - the place where the actual value is written to
} XnCommandI2C;
...
{
  uint32_t count;  // in: number of allocated elements in files array. out:
                   // number of written elements in the array
  XnFwFileEntry *files;
...
{
  uint32_t count;  // in: number of allocated elements in devices array. out:
                   // number of written elements in the array
...
{
  uint32_t count;  // in: number of allocated elements in tests array. out:
                   // number of written elements in the array
  XnBistInfo *tests;
...
typedef struct XnCommandExecuteBist
{
  uint32_t id;
  uint32_t errorCode;
  uint32_t extraDataSize;  // in: number of allocated bytes in extraData. out:
                           // number of written bytes in extraData
  uint8_t *extraData;
...
{
  uint32_t seconds;
  uint32_t endpointCount;  // in: number of allocated bytes in endpoints array.
                           // out: number of written bytes in array
  XnUsbTestEndpointResult *endpoints;
} XnCommandUsbTest;

typedef struct XnCommandGetLogMaskList
{
  uint32_t count;  // in: number of allocated elements in masks array. out:
                   // number of written elements in the array
  XnFwLogMask *masks;
