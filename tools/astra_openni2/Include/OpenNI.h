...
public:
  ...
  */
  ...
  data.
  */
  Array(const T *data, int count) : m_owner(false)
  {
    _setData(data, count);
  }
  ...
  */
  ...
  array
  of specified type.  The optional data ownership flag controls whether the
  primitive
...
public:
  ...
  should, in most
  cases, use the copy constructor to copy an existing valid video mode.  This is
  much less
  error prone that creating and attempting to configure a new VideoMode from
  scratch.
  */
  VideoMode()
  {
  ...
  */
  ...
  function is not recommended.  Instead, use @ref
  SensorInfo::getSupportedVideoModes() to
  ...
  */
  ...
  recommended.
  Instead, use @ref SensorInfo::getSupportedVideoModes() to obtain a list of
  valid
  video modes.
...
A @ref VideoStream object holds one SensorInfo object, describing the sensor
used to produce that stream.

A given SensorInfo object will contain the type of the sensor (Depth, IR or
Color), and
a list of all video modes that the sensor can support.  Each available video
mode will have a single
...
In fact, no
public constructors are provided.  SensorInfo objects should be obtained either
from a Device or @ref VideoStream,
...
public:
  ...
  */
  ...
  the
  recommended method to be used by applications to obtain @ref VideoMode
  objects.
...
@ref VideoStream at a specific time.
The data contained will be a single frame of color, IR, or depth video, along
with associated meta data.

An object of type @ref VideoFrameRef does not actually hold the data of the
frame, but only a reference to it. The
reference can be released by destroying the @ref VideoFrameRef object, or by
calling the @ref release() method. The
...
public:
  ...
  */
  ...
  frame references.
  If this object referenced another frame before calling this method, the
  previous frame will be released.
  @param [in] other Another @ref VideoFrameRef object.
  */
  ...
  */
  ...
  determine whether
  this is an IR, Color or Depth frame.  See the @ref SensorType enumeration for
  all possible return
  ...
  */
  ...
  object describes
  the video mode the sensor was configured to when the frame was produced and
  can be used
  to determine the pixel format and resolution of the data.  It will also
  ...
  */
  ...
  guaranteed to use the same zero.
  This value can therefore be used to compute time deltas between frames from
  the same device,
  ...
  */
  ...
  them.  If frame
  synchronization has been enabled for a device via @ref
  Device::setDepthColorSyncEnabled(), then frame
  ...
  timestamps instead of frame indexes to
  ...
  */
  ...
  the width of the cropping window.  If cropping is not enabled, then this will
  simply be equal to
  ...
  */
  ...
  enabled, this will
  be the length of the cropping window.  If cropping is not enabled, then this
  will simply be equal
  ...
  */
  ...
  becomes invalid, and no method
  should be called other than the assignment operator, or passing this object to
  a @ref VideoStream::readFrame() call.
  */
  void release()
  {
    if (m_pFrame != NULL)
...
Once created, it is used to start data flow
from the device, and to read individual frames of data.  This is the central
class used to obtain data in OpenNI.  It
provides the ability to manually read data in a polling loop, as well as
providing events and a Listener class that can be
...
configuration properties that apply to a specific
stream (rather than to an entire device).  In particular, it is used to control
cropping, mirroring, and video modes.
...
is useful if several components of an application
...
single configuration for the sensor,
...
public:
  ...
  implementation of event driven frame reading.  To use
  it, create a class that inherits from it and implement override the
  onNewFrame() method.  Then, register
  your created class with an active @ref VideoStream using the @ref
  VideoStream::addNewFrameListener() function.  Once this is done, the
  event handler function you implemented will be called whenever a new frame
  becomes available. You may call
  ...
  */
  ...
  object created will be invalid until its create() function
  ...
  */
  ...
  initialized handle.
  This object will not destroy the underlying handle when  @ref destroy() or
  destructor is called
  */
  explicit VideoStream(OniStreamHandle handle)
    : m_stream(NULL), m_sensorInfo(), m_pCameraSettings(NULL), m_isOwner(false)
  {
    _setHandle(handle);
  }
  ...
  a best practice for applications to
  ...
  */
  ...
  You must supply a reference to a
  Device that supplies the sensor type requested.  You can use @ref
  Device::hasSensor() to check whether a
  ...
  */
  ...
  destructor, but it is
  considered a best practice for applications to manually call this function on
  any @ref VideoStream that they
  call create() for.
  */
  inline void destroy();
  ...
  producing this @ref VideoStream.  Note that
  this function will return NULL if the stream has not yet been initialized with
  the create() function.
  ...
  */
  ...
  To avoid blocking, use @ref VideoStream::Listener to implement an event driven
  architecture.  Another
  alternative is to use @ref OpenNI::waitForAnyStream() to wait for new frames
  ...
  */
  ...
  VideoStream::NewFrameListener for
  ...
  */
  Status addNewFrameListener(NewFrameListener *pListener)
  {
    ...
    return (Status)oniStreamRegisterNewFrameCallback(m_stream, pListener->callback, pListener,
                                                     &pListener->m_callbackHandle);
  }
  ...
  */
  Status setFrameBuffersAllocator(FrameAllocator *pAllocator)
  {
    ...
    {
      return (Status)oniStreamSetFrameBuffersAllocator(m_stream, pAllocator->allocateFrameBufferCallback,
  ...
  */
  ...
  it is not
  expected that applications will make direct use of the getProperty function
  very often.
  ...
  */
  ...
  There are convenience functions available for all commonly used properties, so
  it is not
  expected that applications will make direct use of the setProperty function
  very often.
  ...
  */
  ...
  @ref Device::getSensorInfo(), and
  then @ref SensorInfo::getSupportedVideoModes() to obtain a list of valid video
  mode settings for this stream.  Then,
  ...
  */
  ...
  This is most useful
  for getting the minimum possible value that will be reported by a depth
  stream.
  ...
  */
  ...
  expected that
  application code will need this function frequently, as all commonly used
  properties have
...
or a file
device holding a recording from a hardware device.  It offers the ability to
connect to
the device, and obtain information about its configuration and the data streams
it can offer.

It provides the means to query and change all configuration parameters that
apply to the
device as a whole.  This includes enabling depth/color image registration and
frame
synchronization.

Devices are used when creating and initializing @ref VideoStream "VideoStreams"
-- you will need a valid pointer to
a Device in order to use the VideoStream.create() function.  This, along with
configuration, is
...
public:
  ...
  invalid until it is initialized by
  ...
  */
  ...
  handle.
  This object will not destroy the underlying handle when  @ref close() or
  destructor is called
  */
  ...
  */
  ...
  devices
  on the system, or open a specific device selected by passing this function the
  device URI.

  To open any device, simply pass the constant@ref ANY_DEVICE to this function.
  If multiple
  devices are connected to the system, then one of them will be opened.  This
  procedure is most
  useful when it is known that exactly one device is (or can be) connected to
  the system.  In that case,
  requesting a list of all devices and iterating through it would be a waste of
  effort.

  If multiple devices are (or may be) connected to a system, then a URI will be
  required to select
  a specific device to open.  There are two ways to obtain a URI: from a
  DeviceConnected event, or
  by calling @ref OpenNI::enumerateDevices().

  In the case of a DeviceConnected event, the @ref OpenNI::Listener will be
  provided with a DeviceInfo object
  as an argument to its @ref OpenNI::Listener::onDeviceConnected
  ...
  objects, one for each device
  currently available to the system.  The application can then iterate through
  this list and
  select the desired device.  The URI is again obtained via the @ref
  DeviceInfo::getUri() function.
  ...
  */
  ...
  appropriate.  This
  function is currently called by the destructor if not called manually by
  application code, but it
  ...
  */
  ...
  This object can
  be used to access the URI of the device, as well as various USB descriptor
  strings that might
  be useful to an application.

  Note that valid device info will not be available if this device has not yet
  been opened.  If you are
  trying to obtain a URI to open a device, use OpenNI::enumerateDevices()
  instead.
  ...
  */
  ...
  @ref SensorType is
  available on this device.  This allows an application to, for example, query
  for the presence
  ...
  */
  ...
  SensorInfo
  is useful primarily for determining which video modes are supported by the
  sensor.
  ...
  */
  ...
  points in space.  Please see the OpenNi 2.0 Programmer's Guide for more
  information about
  registration.
  ...
  */
  bool isFile() const
  {
    return isPropertySupported(DEVICE_PROPERTY_PLAYBACK_SPEED) &&
           isPropertySupported(DEVICE_PROPERTY_PLAYBACK_REPEAT_ENABLED) && isCommandSupported(DEVICE_COMMAND_SEEK);
  }

  /**
  Used to turn the depth/color frame synchronization feature on and off.  When
  frame synchronization
  is enabled, the device will deliver depth and image frames that are separated
  in time
  by some maximum value.  When disabled, the phase difference between depth and
  image frame
 ...
 * fast forward / rewind / seek), specify whether the playback should be
 *repeated at the end
 ...
 *they are
 * split out into a seperate playback control class.  To use, simply create your
 *file device,
 * create a PlaybackControl, and then attach the PlaybackControl to the file
 *device.
 */
class PlaybackControl
{
public:
   ...
   * presently detaches
   * from its recording automatically, but it is considered a best practice for
   * applications to
   ...
   *30fps, and
   * playback speed is set to 0.5, then the recording will play at 15fps.  If
   *playback speed
   * is set to 2.0, then the recording would playback at 60fps.
   *
   * In addition, there are two "special" values.  A playback speed of 0.0
   *indicates that the
   * playback should occur as fast as the system is capable of returning frames.
   *This is
   * most useful when testing algorithms on large datasets, as it enables
   *playback to be
   ...
   *will only
   * become available when an application manually reads them.  If used in a
   *polling loop,
   * this setting also enables systems to read and process frames limited only
   *by
  ...
  */
  ...
  /**
  * Changes the current repeat mode of the device.  If repeat mode is turned on,
  *then the recording will
  * begin playback again at the beginning after the last frame is read.  If
  *turned off, no more frames
  ...
  */
  ...
  /**
  * Seeks within a VideoStream to a given FrameID.  Note that when this function
  *is called on one
  * stream, all other streams will also be changed to the corresponding place in
  *the recording.  The FrameIDs
  * of different streams may not match, since FrameIDs may differ for streams
  *that are not synchronized, but
  ...
  */
   ...
   *stream.  This is useful
   * both to determine the length of the recording, and to ensure that a valid
   *Frame Index is set when using
 ...
 *OpenNI 2.0
 * application to initialize the SDK and drivers to enable creation of valid
 *device objects.
 *
 ...
 * you to wait for data to become available on any one of a list of streams (as
 *opposed to
 * waiting for data on one specific stream with functions provided by the
 *VideoStream class)
 *
*/
class OpenNI
...
public:
   ...
   *system (ie this event
   * would be triggered when a new sensor is manually plugged into the host
   *system running the
   * application)
   *
   ...
   * OpenNI::addDeviceConnectedListener() function to add your listener object
   *to OpenNI's list of listeners.  Your
   * handler function will then be called whenever the event occurs.  A
   *OpenNI::removeDeviceConnectedListener()
   * function is also provided, if you want to have your class stop listening to
   *these events for any
   * reason.
  */
  ...
  public:
    ...
    * device is removed, if it was opened by a @ref Device object, that object
    *can no longer be
    * used to access the device, even if it was reconnected. Once a device was
    *reconnected,
   ...
   *registering for, and responding to
   ...
   * OpenNI::addDeviceDisconnectedListener() function to add your listener
   *object to OpenNI's list of listeners.  Your
   * handler function will then be called whenever the event occurs.  A
  ...
  */
  ...
  public:
     ...
     * called whenever this event occurs.  When this happens, a pointer to the
     * DeviceInfo
     * object for the newly disconnected device will be supplied.  Note that
     * once a
     * device is removed, if it was opened by a @ref Device object, that object
     * can no longer be
     * used to access the device, even if it was reconnected. Once a device was
   ...
   * OpenNI::addDeviceStateChangedListener() function to add your listener
   *object to OpenNI's list of listeners.  Your
   * handler function will then be called whenever the event occurs.  A
  ...
  */
  ...
  public:
    ...
    * be
    * called whenever this event occurs.  When this happens, a pointer to a
    * DeviceInfo
    * object for the affected device will be supplied, as well as the new
    * DeviceState
  ...
  private:
    ...
    deviceStateChangedCallback(const OniDeviceInfo *pInfo, OniDeviceState state, void *pCookie)
    {
  ...
  */
  ...
  {
    return (Status)oniInitialize(ONI_API_VERSION);  // provide version of API, to
                                                    // make sure proper struct
                                                    // sizes are used
  }
  ...
  *extended error information is maintained
  * on a per-thread basis. Multiple threads do not overwrite each other's last
  *extended error information.
  *
  * The extended error information is cleared on every call to an OpenNI method,
  *so you should call this method
  ...
  */
  static Status waitForAnyStream(VideoStream **pStreams, int streamCount, int *pReadyStreamIndex,
  ...
  */
  static Status addDeviceConnectedListener(DeviceConnectedListener *pListener)
  {
    ...
    return (Status)oniRegisterDeviceCallbacks(&pListener->m_deviceConnectedCallbacks, pListener,
  ...
  */
  static Status addDeviceDisconnectedListener(DeviceDisconnectedListener *pListener)
  {
    ...
    return (Status)oniRegisterDeviceCallbacks(&pListener->m_deviceDisconnectedCallbacks, pListener,
                                              &pListener->m_deviceDisconnectedCallbacksHandle);
  }
  /**
  * Add a listener to the list of objects that receive the event when a device's
  *state changes.  See the
  ...
  */
  static Status addDeviceStateChangedListener(DeviceStateChangedListener *pListener)
  {
    ...
    return (Status)oniRegisterDeviceCallbacks(&pListener->m_deviceStateChangedCallbacks, pListener,
  ...
  * the @ref OpenNI::DeviceConnectedListener class for details on utilizing the
  *events provided by OpenNI.
  *
  ...
  */
  ...
  /**
  * Remove a listener from the list of objects that receive the event when a
  *device is disconnected.  See
  * the @ref OpenNI::DeviceDisconnectedListener class for details on utilizing
  *the events provided by OpenNI.
  *
  * @param pListener Pointer to the Listener to be removed from the list
  * @returns Status code indicating the success or failure of this operation.
  */
  ...
  /**
  * Remove a listener from the list of objects that receive the event when a
  *device's state changes.  See
  * the @ref OpenNI::DeviceStateChangedListener class for details on utilizing
  *the events provided by OpenNI.
  *
  * @param pListener Pointer to the Listener to be removed from the list
  * @returns Status code indicating the success or failure of this operation.
...
depth.  These two systems are referred to as Depth
and World representation.

Depth coordinates are the native data representation.  In this system, the frame
is a map (two dimensional array), and each pixel is
assigned a depth value.  This depth value represents the distance between the
camera plane and whatever object is in the given
pixel. The X and Y coordinates are simply the location in the map, where the
origin is the top-left corner of the field of view.

World coordinates superimpose a more familiar 3D Cartesian coordinate system on
the world, with the camera lens at the origin.
In this system, every point is specified by 3 points -- x, y and z.  The x axis
of this system is along a line that passes
through the infrared projector and CMOS imager of the camera.  The y axis is
parallel to the front face of the camera, and
perpendicular to the x axis (it will also be perpendicular to the ground if the
camera is upright and level).  The z axis
runs into the scene, perpendicular to both the x and y axis.  From the
perspective of the camera, an object moving from
left to right is moving along the increasing x axis.  An object moving up is
moving along the increasing y axis, and an object
...
the CMOS. If the sensor's angular field of view and
resolution are known, then an angular size can be calculated for each pixel.
This is how the conversion algorithms work.  The
dependence of this calculation on FoV and resolution is the reason that a @ref
VideoStream pointer must be provided to these
functions.  The @ref VideoStream pointer is used to determine parameters for the
specific points to be converted.

Since Depth coordinates are a projective, the apparent size of objects in depth
coordinates (measured in pixels)
will increase as an object moves closer to the sensor.  The size of objects in
...
computationally.  It is generally not practical to convert
the entire raw depth map to World coordinates.  A better approach is to have
your computer vision algorithm work in Depth
coordinates for as long as possible, and only converting a few specific points
to World coordinates right before output.
...
public:
  ...
  */
  static Status convertWorldToDepth(const VideoStream &depthStream, float worldX, float worldY, float worldZ,
                                    int *pDepthX, int *pDepthY, DepthPixel *pDepthZ)
  {
    float depthX, depthY, depthZ;
    Status rc = (Status)oniCoordinateConverterWorldToDepth(depthStream._getHandle(), worldX, worldY, worldZ, &depthX,
                                                           &depthY, &depthZ);
    *pDepthX = (int)depthX;
    *pDepthY = (int)depthY;
  ...
  */
  ...
  {
    return (Status)oniCoordinateConverterWorldToDepth(depthStream._getHandle(), worldX, worldY, worldZ, pDepthX,
                                                      pDepthY, pDepthZ);
  }
  ...
  output value, measured in millimeters in World coordinates
  */
  static Status convertDepthToWorld(const VideoStream &depthStream, int depthX, int depthY, DepthPixel depthZ,
                                    float *pWorldX, float *pWorldY, float *pWorldZ)
  {
    return (Status)oniCoordinateConverterDepthToWorld(depthStream._getHandle(), float(depthX), float(depthY),
  ...
  output value, measured in millimeters in World coordinates
  */
  static Status convertDepthToWorld(const VideoStream &depthStream, float depthX, float depthY, float depthZ,
                                    float *pWorldX, float *pWorldY, float *pWorldZ)
  {
    return (Status)oniCoordinateConverterDepthToWorld(depthStream._getHandle(), depthX, depthY, depthZ, pWorldX,
                                                      pWorldY, pWorldZ);
  }

  /**
  For a given depth point, provides the coordinates of the corresponding color
  value.  Useful for superimposing the depth and color images.
  This operation is the same as turning on registration, but is performed on a
  single pixel rather than the whole image.
  ...
  given depth pixel, measured in pixels
  */
  static Status convertDepthToColor(const VideoStream &depthStream, const VideoStream &colorStream, int depthX,
                                    int depthY, DepthPixel depthZ, int *pColorX, int *pColorY)
  {
    return (Status)oniCoordinateConverterDepthToColor(depthStream._getHandle(), colorStream._getHandle(), depthX,
 ...
 *filename where
 * the recording will be stored.  The recorder is then attached to one or more
 *streams.  Once
 * this is complete, the recorder can be told to start recording.  The recorder
 *will store
 * every frame from every stream to the specified file.  Later, this file can be
 *used to
 * initialize a file Device, and used to play back the same data that was
 *recorded.
 *
...
public:
   ...
   * Initializes a recorder. You can initialize the recorder only once. Attempts
       * to intialize more than once will result in an error code being
   *returned.
       *
       * Initialization assigns the recorder to an output file that will be used
   *for
       * recording.  Before use, the @ref attach() function must also be used to
   *assign input
       * data to the Recorder.
       *
   * @param	[in]	fileName	The name of a file which will contain the
   *recording.
       * @returns Status code which indicates success or failure of the
   *operation.
   */
   ...
   *recorder might use
       *	a lossy compression, which means that when the recording will be
   *played-back, there might
  ...
  Status attach(VideoStream &stream, bool allowLossyCompression = false)
  {
    ...
    return (Status)oniRecorderAttachStream(m_recorder, stream._getHandle(), allowLossyCompression);
  }

  /**
   * Starts recording.
       * Once this method is called, the recorder will take all subsequent
   * frames from the attached streams
