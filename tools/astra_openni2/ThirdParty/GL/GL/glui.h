...
public:
  ...
  /** Return true if this control should redraw itself immediately (front
     buffer);
  ...
  */
  ...
  /** Switch to the appropriate draw buffer now.  Returns the old draw buffer.
     This routine should probably only be called from inside the
...
public:
  ...
  GLUI *glui;        /**< Our containing event handler (NEVER NULL during event
                        processing!) */
...
public:
  ...
  /**
   Create a new button.
    ...
    @param callback Optional callback function, taking either the int ID or
   control.
  */
...
public:
  ...
  /**
   Create a new checkbox object.
    ...
    @param value_ptr Optional integer value to attach to this checkbox.  When
   the
       checkbox is checked or unchecked, *value_ptr will also be changed. ("Live
   Vars").
    @param id Optional ID number, to pass to the optional callback function.
    @param callback Optional callback function, taking either the int ID or
   control.
...
public:
  /**
   Create a new panel.  A panel groups together a set of related controls.
    ...
    @param type Optional style to display the panel with--GLUI_PANEL_EMBOSSED by
   default.
        GLUI_PANEL_RAISED causes the panel to appear higher than the
   surroundings.
...
public:
  /**
   Create a new list of files the user can select from.
    ...
    @param frame_type Optional style to display the panel
   with--GLUI_PANEL_EMBOSSED by default.
        GLUI_PANEL_RAISED causes the panel to appear higher than the
   surroundings.
        GLUI_PANEL_NONE causes the panel's outline to be invisible.
    @param id Optional ID number, to pass to the optional callback function.
   ...
   control.
  */
  GLUI_FileBrowser(GLUI_Node *parent, const char *name, int frame_type = GLUI_PANEL_EMBOSSED, int user_id = -1,
...
public:
  /**
   ...
   like a panel, but can be collapsed to just the name.
    ...
    @param open Optional boolean.  If true (the default), the rollout's controls
   are displayed.
...
public:
  ...
  {
    return main_panel->add_control(control);
  }
  ...
  GLUI_RadioGroup *add_radiogroup_to_panel(GLUI_Panel *panel, int *live_var = NULL, int user_id = -1,
                                           GLUI_CB callback = GLUI_CB());
  ...
  GLUI_Listbox *add_listbox_to_panel(GLUI_Panel *panel, const char *name, int *live_var = NULL, int id = -1,
                                     GLUI_CB callback = GLUI_CB());

  GLUI_Rotation *add_rotation(const char *name, float *live_var = NULL, int id = -1, GLUI_CB callback = GLUI_CB());
  GLUI_Rotation *add_rotation_to_panel(GLUI_Panel *panel, const char *name, float *live_var = NULL, int id = -1,
                                       GLUI_CB callback = GLUI_CB());

  GLUI_Translation *add_translation(const char *name, int trans_type, float *live_var = NULL, int id = -1,
                                    GLUI_CB callback = GLUI_CB());
  GLUI_Translation *add_translation_to_panel(GLUI_Panel *panel, const char *name, int trans_type,
                                             float *live_var = NULL, int id = -1, GLUI_CB callback = GLUI_CB());

  GLUI_Checkbox *add_checkbox(const char *name, int *live_var = NULL, int id = -1, GLUI_CB callback = GLUI_CB());
  GLUI_Checkbox *add_checkbox_to_panel(GLUI_Panel *panel, const char *name, int *live_var = NULL, int id = -1,
                                       GLUI_CB callback = GLUI_CB());
  ...
  GLUI_EditText *add_edittext(const char *name, int data_type = GLUI_EDITTEXT_TEXT, void *live_var = NULL, int id = -1,
                              GLUI_CB callback = GLUI_CB());
  GLUI_EditText *add_edittext_to_panel(GLUI_Panel *panel, const char *name, int data_type = GLUI_EDITTEXT_TEXT,
                                       void *live_var = NULL, int id = -1, GLUI_CB callback = GLUI_CB());
  GLUI_EditText *add_edittext(const char *name, GLUI_String &live_var, int id = -1, GLUI_CB callback = GLUI_CB());
  GLUI_EditText *add_edittext_to_panel(GLUI_Panel *panel, const char *name, GLUI_String &live_var, int id = -1,
                                       GLUI_CB callback = GLUI_CB());

  GLUI_Spinner *add_spinner(const char *name, int data_type = GLUI_SPINNER_INT, void *live_var = NULL, int id = -1,
                            GLUI_CB callback = GLUI_CB());
...
public:
  ...
  GLUI_EditText(GLUI_Node *parent, const char *name, int text_type = GLUI_EDITTEXT_TEXT, int id = -1,
                GLUI_CB callback = GLUI_CB());
...
protected:
  ...
  void common_construct(GLUI_Node *parent, const char *name, int data_type, int live_type, void *live_var, int id,
                        GLUI_CB callback);
};
...
public:
  ...
  GLUI_CommandLine(GLUI_Node *parent, const char *name, void *live_var = NULL, int id = -1,
                   GLUI_CB callback = GLUI_CB());
...
public:
  ...
  GLUI_Spinner(GLUI_Node *parent, const char *name, int data_type = GLUI_SPINNER_INT, int id = -1,
               GLUI_CB callback = GLUI_CB());
  ...
  GLUI_Spinner(GLUI_Node *parent, const char *name, int data_type, void *live_var, int id = -1,
               GLUI_CB callback = GLUI_CB());
  // Deprecated constructor
  GLUI_Spinner(void)
  {
...
public:
  /* GLUI Textbox - JVK */
  GLUI_TextBox(GLUI_Node *parent, GLUI_String &live_var, bool scroll = false, int id = -1,
               GLUI_CB callback = GLUI_CB());
...
public:
  // Constructor, no live var
  GLUI_Scrollbar(GLUI_Node *parent, const char *name, int horz_vert = GLUI_SCROLL_HORIZONTAL,
                 int data_type = GLUI_SCROLL_INT, int id = -1, GLUI_CB callback = GLUI_CB()
                 /*,GLUI_Control *object = NULL*/
                 /*,GLUI_InterObject_CB obj_cb = NULL*/
                 );

  // Constructor, int live var
  GLUI_Scrollbar(GLUI_Node *parent, const char *name, int horz_vert, int *live_var, int id = -1,
                 GLUI_CB callback = GLUI_CB()
                 /*,GLUI_Control *object = NULL*/
                 /*,GLUI_InterObject_CB obj_cb = NULL*/
                 );
  ...
  // Constructor, float live var
  GLUI_Scrollbar(GLUI_Node *parent, const char *name, int horz_vert, float *live_var, int id = -1,
                 GLUI_CB callback = GLUI_CB()
                 /*,GLUI_Control *object = NULL*/
                 /*,GLUI_InterObject_CB obj_cb = NULL*/
                 );
...
protected:
  void common_init(void);
  void common_construct(GLUI_Node *parent, const char *name, int horz_vert, int data_type, void *live_var, int id,
                        GLUI_CB callback
                        /*,GLUI_Control *object
                        ,GLUI_InterObject_CB obj_cb*/
                        );
...
public:
  ...
  GLUI_Translation(GLUI_Node *parent, const char *name, int trans_type, float *live_var = NULL, int id = -1,
                   GLUI_CB callback = GLUI_CB());
