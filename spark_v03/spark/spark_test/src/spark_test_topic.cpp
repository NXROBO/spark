...
public:
	...
	bool point_clound_state, depth_image_state, rgb_image_state,
			gyro_state,odom_state,ir_bumper_cliff_state;
	...
	TopicSubscribeTest(ros::NodeHandle in_nh) {
		...
		test_topic_thread_ = new boost::thread(
				boost::bind(&TopicSubscribeTest::testTopicStatus, this));
	}
	...
	bool testPointCloudTopic() {
		ros::Subscriber pcloud_sub = nhandle.subscribe<PointCloud>(
				"/camera/depth/points", 1, &TopicSubscribeTest::pointCloudCb,
				this);
		point_clound_state = false;
		ros::Rate rate(10);
		int max_wait_time = 5;
	...
	bool testDepthImageTopic() {
		ros::Subscriber sub = nhandle.subscribe<sensor_msgs::Image>(
				"/camera/depth/image", 1, &TopicSubscribeTest::depthImageCb,
				this);
		depth_image_state = false;
		ros::Rate rate(10);
		int max_wait_time = 5;
	...
	bool testRgbImageTopic() {
		ros::Subscriber sub = nhandle.subscribe<sensor_msgs::Image>(
				"/camera/rgb/image_raw", 1, &TopicSubscribeTest::rgbImageCb,
				this);
		rgb_image_state = false;
		ros::Rate rate(10);
		int max_wait_time = 5;
	...
	//test gyro topic
	bool testGyroTopic() {
		ros::Subscriber sub = nhandle.subscribe<spark_base::GyroMessage>(
				"/spark_base/gyro", 1, &TopicSubscribeTest::gyroCb, this);
		gyro_state = false;
		ros::Rate rate(10);
		int max_wait_time = 5;
		double prev_time = ros::Time().now().toSec();
	...
	//test odom topic
	bool testOdomTopic() {
		ros::Subscriber sub = nhandle.subscribe<nav_msgs::Odometry>("/odom", 1,
				&TopicSubscribeTest::odomCb, this);
		odom_state = false;
		ros::Rate rate(10);
		int max_wait_time = 5;
		double prev_time = ros::Time().now().toSec();
